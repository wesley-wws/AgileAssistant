...
public class DeckEFRepository : IDeckRepository
{
    ...
    {
        return _dBContext.Decks.Include(d => d.Pokers).AsNoTracking().ToListAsync(cancellationToken);
    }

    public Task<Deck?> FindAsync(Guid id, CancellationToken cancellationToken)
    {
        return _dBContext.Decks.Include(d => d.Pokers).AsNoTracking().FirstOrDefaultAsync(d => d.Id == id, cancellationToken);
    }
}
